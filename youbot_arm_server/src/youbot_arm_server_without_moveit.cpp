...
public:
	...
	typedef std::vector<JointAngles> JointAglesList;
	 ...
	 */
	void execute_trajectory (const std::vector<JointAngles> &JointAnglesList) {

		int size_trajectory = JointAnglesList.size();
		...
		for (std::size_t itp = 0; itp < size_trajectory; itp++) {
			...
			for (std::size_t itj = 0; itj < no_joints; itj++) {
				...
				goal_trajectory.trajectory.points[itp].positions[itj] = JointAnglesList.at(itp).at(itj);
...
//				ROS_WARN("Kinematic solution does not yield a valid plan. Check for collisions with sensors");
